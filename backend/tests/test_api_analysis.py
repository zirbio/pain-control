...
from backend.db.models import AppleHealthRecord, DailyEntry, PainRecord
...
def client_with_data(tmp_path):
    ...
    for i in range(30):
        ...
        entry.apple_health_records.append(
            AppleHealthRecord(sleep_hours=sleep, steps=int(np.random.normal(6000, 2000)))
        )
        session.add(entry)
    session.commit()
    session.close()
...
def test_lag_correlation_endpoint(client_with_data):
    response = client_with_data.get(
        "/api/analysis/lag-correlation?target=pain_max&variable=sleep_hours&max_lag=2"
    )
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 5
...
def test_report_endpoint(client_with_data):
    response = client_with_data.get(
        "/api/analysis/report?start_date=2026-03-01&end_date=2026-03-30"
    )
    assert response.status_code == 200
    data = response.json()
    assert "pain" in data
...
def test_report_with_no_data(client_with_data):
    ...
    response = client_with_data.get(
        "/api/analysis/report?start_date=2020-01-01&end_date=2020-01-31"
    )
    assert response.status_code == 200
    data = response.json()
    assert "error" in data
