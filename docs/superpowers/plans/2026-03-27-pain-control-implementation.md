...
def test_default_settings():
    ...
    assert settings.WEATHER_LOCATION == "Madrid"
...
class Settings(BaseSettings):
    ...
    WEATHER_LOCATION: str = "Madrid"
    WEATHER_LAT: float = 40.4168
    WEATHER_LON: float = -3.7038
...
def test_create_entry(client):
    response = client.post("/api/entries", json={
        ...
        "medication_records": [{"name": "Captor", "dose": "75mg", "time_taken": "08:00", "effectiveness": 7}],
...
Run:
```bash
cd /Users/silvio_requena/Code/pain-control && npx create-next-app@latest dashboard --typescript --tailwind --eslint --app --src-dir --import-alias "@/*" --no-turbopack
```
...
- Location colors: lumbar=`#C4512A`, tobillo=`#D4A03A`, ciática=`#A8B86A`
...
- medicación/Captor → medication_effectiveness
...
curl -X POST http://localhost:8000/api/entries \
  ...
  -d '{
    ...
    "medication_records": [{"name": "Captor", "dose": "75mg tramadol + paracetamol", "time_taken": "08:00", "effectiveness": 7}],
